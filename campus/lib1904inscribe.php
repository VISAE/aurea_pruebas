...
TB.even02finfecha, TB.even02finhora, TB.even02finminuto, "" AS even13nombre, TB.even02idcertificado, TB.even02modalidad, TB.even02formainscripcion
...
AND STR_TO_DATE(TB.even02inifecha,"%d/%m/%Y") '.$operadorFecha.' DATE(NOW()) ';
	...
	$sSQL=$sSQL.'SELECT '.$sSQLadd1.', T3.even01nombre, T4.even41titulo, T5.even14nombre, TB.even02nombre, 
TB.even02lugar, TB.even02inifecha, TB.even02inihora, TB.even02iniminuto, 
TB.even02finfecha, TB.even02finhora, TB.even02finminuto, T6.even13nombre, TB.even02idcertificado, TB.even02modalidad, TB.even02formainscripcion
		...
		if ($babierta) {
            if ($aParametros[104] == 'bfuturo') {
                if ($filadet['even02modalidad']==0) {
                    $sLink='<a href="javascript:cambiaEstadoEventoId('.$filadet[$sSQLadd1] . ','. $aParametros[103] . ',\'' . $aParametros[104] . '\', 0, '. $filadet['even02formainscripcion'].')" class="lnkresalte">'.'Inscribirme Presencial'.'</a>';
                } else if ($filadet['even02modalidad']==1) {
                    $sLink='<a href="javascript:cambiaEstadoEventoId('.$filadet[$sSQLadd1] . ','. $aParametros[103] . ',\'' . $aParametros[104] . '\',1, '. $filadet['even02formainscripcion'].')" class="lnkresalte">'.'Inscribirme Virtual'.'</a>';
                } else if ($filadet['even02modalidad']==2) {
                    $sLink='<a href="javascript:cambiaEstadoEventoId('.$filadet[$sSQLadd1] . ','. $aParametros[103] . ',\'' . $aParametros[104] . '\',0, '. $filadet['even02formainscripcion'].')" class="lnkresalte">'.'Inscribirme Presencial'.'</a>'.
                           '<a href="javascript:cambiaEstadoEventoId('.$filadet[$sSQLadd1] . ','. $aParametros[103] . ',\'' . $aParametros[104] . '\',1, '. $filadet['even02formainscripcion'].')" class="lnkresalte">'.'Inscribirme Virtual'.'</a>';
			    }
            } else {
                $sLink = '<a href="javascript:cambiaEstadoEventoId('. $filadet[$sSQLadd1] . ','. $aParametros[103] . ',\'' . $aParametros[104] . '\',-1, '. $filadet['even02formainscripcion'].')" class="lnkresalte">' . $ETI[$etiLnk] . '</a>';
            }
		}
		$res=$res.'<tr'.$sClass.'>
    ...
    switch ($aParametros[105]) {
        case 'futuro':
            if($aParametros[107]==0) {
                $sSQL = 'DELETE FROM even04eventoparticipante 
                        WHERE even04id = '.$aParametros[103];
            } else {
        ...
        case 'bfuturo':
            ...
            if($aParametros[107]==0) {
                ...
                $sSQL = 'INSERT INTO even04eventoparticipante (even04idevento, even04idparticipante, even04id, even04estadoasistencia)
                        VALUES ('.$aParametros[103].','.$aParametros[104].','.$even04id.','.$estado.')';
            } else {
                $sSQL = 'UPDATE even04eventoparticipante 
                        SET even04estadoasistencia = ' . $estado . ' 
                        WHERE even04idevento = ' . $aParametros[103] . ' AND even04idparticipante =' . $aParametros[104];
            }
            break;
        }
